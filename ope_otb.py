...
def track_otb(tracker_type,dataset):
    ...
    for video in dataset:
        ...
        elif tracker_type=='Staple_MIX':
            tracker=Staple(config=staple_config.StapleConfig(),mix=1)
        elif tracker_type=='Staple-CA':
            tracker=Staple(config=staple_config.StapleCAConfig())
        elif tracker_type=='Staple-CA_MIX':
            tracker=Staple(config=staple_config.StapleConfig(),mix=1)
        elif tracker_type=='KCF':
            tracker=KCF(features='hog',kernel='gaussian')
        elif tracker_type=='KCF_MIX':
            tracker=KCF(features='hog',kernel='gaussian',mix=1)
        elif tracker_type=='DCF':
            tracker=KCF(features='hog',kernel='linear')
        elif tracker_type=='DCF_MIX':
            tracker=KCF(features='hog',kernel='linear',mix=1)
        ...
        if os.path.exists(result_path) and checkexists:
            continue
