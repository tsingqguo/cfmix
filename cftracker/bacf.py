...
class BACF(BaseCF):
    def __init__(self, config=BACFConfig(),mix=-1):
        super(BACF).__init__()
    ...
    def init(self,first_frame,bbox):
        ...
        if self.mix>0:
            model_xfs = []
            model_xfs.append(self.model_xf)

            for _ in range(self.mix):
                augframe = first_frame.astype(np.float32) / 255
                augframe = augmix(augframe)*255

                pixels = self.get_sub_window(augframe, self._center, model_sz=self.crop_size,
                                             scaled_sz=(int(np.round(self.crop_size[0] * self.current_scale_factor)),
                                                        int(np.round(self.crop_size[1] * self.current_scale_factor))))
                feature = self.extract_hc_feture(pixels, cell_size=self.feature_ratio)
                model_xf = fft2(self._window[:, :, None] * feature)

                model_xfs.append(model_xf)

            model_xfs = np.stack(model_xfs)

            self.model_xf = model_xfs.mean(axis=0)
