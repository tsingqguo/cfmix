...
class Staple(BaseCF):
    def __init__(self, config, mix=-1):
    ...
    def init(self, first_frame, bbox):
        ...
        if self.mix>0:
            hf_nums = []
            hf_dens = []
            hf_nums.append(self.hf_num)
            hf_dens.append(self.hf_den)

            for _ in range(self.mix):
                augframe = first_frame.astype(np.float32) / 255
                augframe = augmix(augframe)*255
                im_patch_bg = self.get_sub_window(augframe, self._center, self.norm_bg_area, self.bg_area)
                xt = self.get_feature_map(im_patch_bg, self.hog_cell_size)
                xt = self._window[:, :, None] * xt
                xtf = fft2(xt)
                if self.use_ca:
                    ...
                    for j in range(len(self.offset)):
                        ...
                        xtfn = fft2(xtn)
                        sum_kfn += np.conj(xtfn) * xtfn
                    hf_num = self.yf[:, :, None] * np.conj(xtf)
                    hf_den = np.conj(xtf) * xtf + self.lambda_ + self.lambda_2 * sum_kfn
                else:
                ...
                hf_nums.append(hf_num)
                hf_dens.append(hf_den)

            hf_nums = np.stack(hf_nums)
            hf_dens = np.stack(hf_dens)

            self.hf_num = hf_nums.mean(axis=0)
            self.hf_den = hf_dens.mean(axis=0)
    ...
    def update(self, current_frame, vis=False):
        ...
        if self.mix>0:
            hf_nums = []
            hf_dens = []
            hf_nums.append(new_hf_num)
            hf_dens.append(new_hf_den)
            ...
            new_hf_num = hf_nums.mean(axis=0)
            new_hf_den = hf_dens.mean(axis=0)
