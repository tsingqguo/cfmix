...
from lib.augmix import augment_and_mix as augmix


class KCF(BaseCF):
    def __init__(self, padding=1.5, features='gray', kernel='gaussian',mix=-1):
        super(KCF).__init__()
    ...
    def init(self,first_frame,bbox):
        ...
        if self.mix>0:
            alphaf = []
            alphaf.append(self.alphaf)
            for _ in range(self.mix):
                augframe = first_frame.astype(np.float32) / 255
                augframe = augmix(augframe)*255

                if self.features == 'gray' or self.features == 'color':
                    augframe = augframe.astype(np.float32) / 255
                    augx = self._crop(augframe, self._center, (w, h))
                    augx = augx - np.mean(augx)
                elif self.features == 'hog':
                    augx = self._crop(augframe, self._center, (w, h))
                    augx = cv2.resize(augx, (self.window_size[0] * self.cell_size, self.window_size[1] * self.cell_size))
                    augx = extract_hog_feature(augx, cell_size=self.cell_size)
                elif self.features == 'cn':
                    augx = cv2.resize(augframe,
                                   (self.window_size[0] * self.cell_size, self.window_size[1] * self.cell_size))
                    augx = extract_cn_feature(augx, self.cell_size)
                else:
                    raise NotImplementedError
                augxf = fft2(self._get_windowed(augx, self._window))
                self.init_response_center = (0, 0)
                augalphaf = self._training(augxf, self.yf)
                alphaf.append(augalphaf)
            alphaf = np.stack(alphaf)
            self.alphaf = alphaf.mean(axis=0)
    ...
    def update(self,current_frame,vis=False):
        ...
        alphaf = self._training(new_xf, self.yf, kernel=self.kernel)

        if self.mix>0:
            alphafs = []
            alphafs.append(alphaf)
            for _ in range(self.mix):
                augframe = current_frame.astype(np.float32) / 255
                augframe = augmix(augframe)*255

                if self.features == 'gray' or self.features == 'color':
                    augframe = augframe.astype(np.float32) / 255
                    augx = self._crop(augframe, self._center, (self.w, self.h))
                    augx = augx - np.mean(augx)
                elif self.features == 'hog':
                    augx = self._crop(augframe, self._center, (self.w, self.h))
                    augx = cv2.resize(augx, (self.window_size[0] * self.cell_size, self.window_size[1] * self.cell_size))
